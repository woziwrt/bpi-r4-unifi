...
# Usage: sh install-nvme-unifi.sh       › prod (Samsung 1TB, p3=30GB)
#        sh install-nvme-unifi.sh test  › test (Patriot 128GB, p3=15GB)
...
if [ "$1" = "test" ]; then
    P3_SIZE="15G"
    PROFILE="test (Patriot 128GB)"
else
    P3_SIZE="30G"
    PROFILE="prod (Samsung 1TB)"
fi
...
case "$USE_LOCAL" in
    ...
    *)
        ...
        ITB_URL="${BASE_URL}/openwrt-mediatek-filogic-bananapi_bpi-r4-squashfs-sysupgrade.itb"
        ...
        if ! wget -q --spider "$ITB_URL" 2>/dev/null; then
            ...
            printf "       Check: https://github.com/${GH_USER}/${GH_REPO}/releases\n\n"
            exit 1
        fi
        printf "        OK -- release available\n\n"

        printf "        Downloading itb...\n"
        wget -O "$ITB" "$ITB_URL"
        if [ $? -ne 0 ] || [ ! -s "$ITB" ]; then
            printf "\n${RED}ERROR: Download of itb failed.${NC}\n\n"
            rm -f "$ITB"
            exit 1
        fi
        printf "        OK -- itb downloaded\n\n"
...
printf "        Creating protect partition (p4)...\n"
...
# Download unifi-setup.sh to p3 so it's ready after NVMe boot
if [ -n "$SETUP_URL" ]; then
    printf "        Downloading unifi-setup.sh to p3...\n"
